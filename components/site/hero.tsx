      ...
      <div className="container-shell relative grid min-h-[calc(100vh-4rem)] items-center gap-10 pb-24 pt-24 md:pb-28 md:pt-28 lg:grid-cols-[minmax(0,1fr)_minmax(340px,0.72fr)]">
        <div className="max-w-3xl animate-reveal space-y-7">
          <p className="inline-flex max-w-full items-center gap-2 rounded-full border border-white/30 bg-white/12 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] backdrop-blur">
            <Sparkles className="size-4 shrink-0" aria-hidden /> {text.hero.eyebrow}
          </p>
          <div className="space-y-5">
            <h1 className="text-5xl font-semibold leading-[1.06] md:text-7xl">{text.hero.title}</h1>
            <p className="max-w-2xl text-lg leading-8 text-white/86 md:text-xl">{text.hero.description}</p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg">
              <Link href="/studios">{text.hero.primaryCta} <ArrowRight aria-hidden /></Link>
            </Button>
            <Button asChild variant="outline" size="lg" className="border-white/40 bg-white/12 text-white hover:bg-white/20">
              <Link href="/reservation"><CalendarDays aria-hidden /> {text.hero.secondaryCta}</Link>
            </Button>
          </div>
        </div>
        ...
        <div className="hidden justify-self-end rounded-lg border border-white/24 bg-white/14 p-5 shadow-soft backdrop-blur-xl lg:block lg:w-full lg:max-w-md">
          <div className="relative aspect-[4/5] overflow-hidden rounded-md bg-white/10">
            <Image
              ...
              alt="Premium Korean wedding studio sample"
              fill
          ...
          <div className="mt-5 grid gap-3 text-sm text-white/86">
            <span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-primary" aria-hidden /> {text.hero.studioMatching}</span>
            <span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-primary" aria-hidden /> {text.hero.destinationPlanning}</span>
            <span className="flex items-center gap-2"><CheckCircle2 className="size-4 text-primary" aria-hidden /> {text.hero.conciergeSupport}</span>
          </div>
        </div>
      </div>
