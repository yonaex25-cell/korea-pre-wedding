        ...
        <div className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
          ...
          <div className="space-y-6 rounded-lg border border-border bg-white p-6 shadow-soft">
            <div className="space-y-3">
              ...
              <p className="flex items-center gap-2 text-muted-foreground"><MapPin className="size-4" aria-hidden /> {studio.region}</p>
              {typeof detail.rating === "number" ? (
                <p className="flex items-center gap-2 font-semibold text-gold-700"><Star className="size-4 fill-gold-500" aria-hidden /> {detail.rating} rating</p>
              ) : null}
            </div>
            <p className="leading-8 text-muted-foreground">{studio.description}</p>
            <div className="grid gap-3 border-y border-border py-5 sm:grid-cols-2">
              {typeof detail.priceFrom === "number" ? (
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Price from</p>
                  <p className="text-2xl font-semibold text-ink">{formatJPY(detail.priceFrom)}</p>
                </div>
              ) : null}
              {destinations.length ? (
                <div>
                  <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Destinations</p>
                  <p className="font-medium text-ink">{destinations.join(" / ")}</p>
                </div>
              ) : null}
            </div>
            <Button asChild size="lg" className="w-full">
              <Link href={"/reservation?studio=" + reservationSlug}>Request this studio <ArrowRight aria-hidden /></Link>
            </Button>
          </div>
        </div>
        ...
        {services.length ? (
          <section className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="space-y-3">
              ...
              <h2 className="text-3xl font-semibold text-ink">Package highlights</h2>
            </div>
        ...
        {faqs.length ? (
          <section className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
            <div className="space-y-3">
              ...
              <h2 className="text-3xl font-semibold text-ink">Questions for this studio</h2>
            </div>
            <FaqList faqs={faqs} />
          </section>
        ) : null}
        ...
        {reviews.length ? (
          <section className="space-y-8">
            <div className="space-y-3">
              ...
              <h2 className="text-3xl font-semibold text-ink">Couple feedback</h2>
            </div>
